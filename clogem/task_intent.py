...
def build_prerequisite_first_prompt(task: str, mem_block: str) -> str:
    ...
    return (
        "You are Clogem's assistant. The user wrote a message that asks for information, "
        ...
        "If they asked about Google Stitch + MCP in clogem: mention Node/npx, "
        "`GOOGLE_CLOUD_PROJECT`, `gcloud auth application-default login`, and that clogem can "
        "spawn `npx stitch-mcp` per README (MCP is on by default; `CLOGEM_STITCH_MCP=0` disables it) — "
        "stay consistent with the project's documented behavior.\n"
